package courtreferences.nigeria;
...
public class NigerianCourtDocument extends CourtDocument implements ProcessCitations {
	/*
	 ...
	 * 
	 */
	public NigerianCourtDocument(){
	}
	
	public NigerianCourtDocument(String countryName, String courtName, String processedUserName, String sourceFileName){
	...
	public int extractCaseId(String CourtName,String pdfFileContent){
		//if(CourtName.equals("Supreme Court")){
	...
	/* Extracts Participants name for each Nigerian case document	*/
	...
	public void extractParticipants(String CourtName,String pdfFileContent){
		if(CourtName.equals("Supreme Court")){
			...
			this.setParticipantsName(extractCNSTCRTParticipants(pdfFileContent).get(0).toString());
		}
		else{
	...
	/* Extracts decision date for each Nigerian case document	*/
	...
	/* Extracts heard date for each Nigerian case document	*/
	...
	/* Extracts citations for each Nigerian case document	*/
	...
	/* Extracts Case id for Nigerian Supreme Court case document*/
	
	public String extractCNSTCRTCaseId(String pdfFileContent){
		String caseidString = "SC\\d{2,3}\\/\\d{2,4}"; 
		Pattern caseidPattern = Pattern.compile(caseidString,Pattern.CASE_INSENSITIVE);
		Matcher caseidMatcher = caseidPattern.matcher(pdfFileContent);
		caseidMatcher.find();
		String caseId = null;
	...
	/* Extracts citations for Nigerian Constitutional Court case document*/
	...
	public void extractCNSTCRTCitations(List<String> fileContent){
		...
		for(String pageContent : fileContent){
			...
			while(true){
				...
				NigerianCitations currentCitationObj = new NigerianCitations(this.getCaseId(),this.getCountryName(),this.getCourtName(),currentCitationId,currCitationString.trim(),currentPageNo);
			...
			PageContent currentPage = new NigerianPageContent(currentPageNo, pageContent.toLowerCase());
	...
	public void extractCitationsFromPageBodyContent(){
		for(PageContent bodyContent : this.getDocumentPages()){
			NigerianCitations citationObj = new NigerianCitations();
	...
	/* Extracts the participants name from Nigerian Supreme Court case document*/

	public List<String> extractCNSTCRTParticipants(String pdfFileContent){
		// Participants appear between these keywords
		String pattern3 = "Between";
		String pattern4 = "Judgment";
		
		//participants 
		Pattern participantsPattern = Pattern.compile(Pattern.quote(pattern3) + "(.*?)" + Pattern.quote(pattern4),
                Pattern.DOTALL);
		Matcher participantsMatcher = participantsPattern.matcher(pdfFileContent);
		List<String> participants = new ArrayList<String>();
		while (participantsMatcher.find()) {
		  participants.add(participantsMatcher.group(1));
	...
	int checkCaseIdPattern(String inputString, int startIndex, int option){
		String caseidPatternString = "([A-Z][A-Z]([A-Z]\\d{2,3})?(\\d{2,3})?[/]\\d{2,4})";
	...
	int checkDecisionDatePattern(String inputString, int startIndex, int opt){
		String decisionDatePatternString = "([1-9]|[12][0-9]|3[01])([t][h]|[r][d]|[s][t]|[n][d])\\s([Dd][Aa][Yy]\\s[o][f])\\s(Jan(uary)?|Feb(ruary)?|Mar(ch)?|Apr(il)?|May|Jun(e)?|Jul(y)?|Aug(ust)?|Sep(tember)?|Oct(ober)?|Nov(ember)?|Dec(ember)?)[- /.]?\\s(1[9][0-9][0-9]|2[0-9][0-9][0-9])";
	...
	int checkDatePattern(String inputString, int startIndex, int opt){
		String datePatternString = "([1-9]|[12][0-9]|3[01])([t][h]|[r][d]|[s][t]|[n][d])\\s([Dd][Aa][Yy]\\s[o][f])\\s(Jan(uary)?|Feb(ruary)?|Mar(ch)?|Apr(il)?|May|Jun(e)?|Jul(y)?|Aug(ust)?|Sep(tember)?|Oct(ober)?|Nov(ember)?|Dec(ember)?)[- /.]?\\s(1[9][0-9][0-9]|2[0-9][0-9][0-9])";
	...
	String extractCNSTCRTDecisionDate(String pdfFileContent){
		//Decision date occurs between these participants
		String decisionDate  = null;
		String pattern1 = "NIGERIA";
		String pattern2 = "Before";
		
		//decision date
		Pattern datePattern = Pattern.compile(Pattern.quote(pattern1) + "(.*?)" + Pattern.quote(pattern2),
                Pattern.DOTALL);
		Matcher dateMatcher = datePattern.matcher(pdfFileContent);
		while (dateMatcher.find()) {
		  String dateString = dateMatcher.group(1).split("the")[1];
		  decisionDate = dateString.replace("Day of ", "");
	...
	/* Searches for the Heard Date Pattern in Nigerian Constitutional court. If not found, the default date is set */
