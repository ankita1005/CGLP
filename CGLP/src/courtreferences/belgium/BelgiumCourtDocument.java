package courtreferences.belgium;
...
public class BelgiumCourtDocument extends CourtDocument implements ProcessCitations {
	/*
	 ...
	 * 
	 */
	public BelgiumCourtDocument(){
	}
	
	public BelgiumCourtDocument(String countryName, String courtName, String processedUserName, String sourceFileName){
	...
	public int extractCaseId(String CourtName,String pdfFileContent){
		if(CourtName.equals("Constitutional Court" )|| CourtName.equals("Supreme Court of Justice") || CourtName.equals("Other Court")){
	...
	public String extractCNSTCRTCaseId(String pdfFileContent){
		...
		if(startIndex != -1 && endIndex != -1){
			String caseID = "N° "+pdfFileContent.substring(startIndex+9,endIndex);
			//System.out.println("temp case id "+caseIDtemp);
			//String caseID = pdfFileContent.substring((caseIDtemp.indexOf("n°")),endIndex);
			System.out.println("Case ID: "+caseID);
			//String caseID = pdfFileContent.substring(startIndex,endIndex);
			caseID = caseID.replaceAll("/", ":");
			return caseID;
		}
		return null;
	}
		...
		for(String pageContent : fileContent){
			...
			while(true){
				...
				if(citationStartIndex == -1)
					break;
				firstCitationIndex = citationStartIndex; 
				...
				BelgiumCitations currentCitationObj = new BelgiumCitations(this.getCaseId(),this.getCountryName(),this.getCourtName(),currentCitationId,currCitationString.trim(),currentPageNo);
			...
			//skannan: changed the condition variable from firstCitationIndex to citationStartIndex
			String currCitationString = pageContent.replaceAll("\n", " ");
			if(citationStartIndex != -1){
				currCitationString = pageContent.substring(0,citationStartIndex).replaceAll("\n", " ");
			}
			...
			PageContent currentPage = new BelgiumPageContent(currentPageNo, pageContent.toLowerCase());
	...
	public void extractCitationsFromPageBodyContent(){
		for(PageContent bodyContent : this.getDocumentPages()){
			BelgiumCitations citationObj = new BelgiumCitations();
	...
	int checkCaseIdPattern(String inputString, int startIndex, int option){
		String caseidPatternString = "([A][r][r][ê][t]\\s[n][°]\\s\\d{1,3}\\/\\d{1,4})";
	...
	int checkDecisionDatePattern(String inputString, int startIndex, int opt){
		//String decisionDatePatternString = "([A][r][r][ê][t]\\s[n][°]\\s\\d{1,3}\\/\\d{1,4})(\\s)?(\\n)?[d][u]\\s\\d{1,2}\\s\\w{1,20}\\b\\s\\d{4}";
		//String decisionDatePatternString = "[d][u]\\s\\d{1,2}\\s\\w{1,10}\\b\\s\\d{4}";
	...
	String extractCNSTCRTDecisionDate(String pdfFileContent){
		...
		String decisionDateTemp  = null;
		String month = null;
		int startIndex = 0;
		int endIndex = 0;
		startIndex = checkDecisionDatePattern(pdfFileContent, startIndex, 0);
		endIndex = checkDecisionDatePattern(pdfFileContent,startIndex,1);
		
		if(endIndex != -1 && startIndex != -1)
		{
			decisionDateTemp = pdfFileContent.substring(startIndex,endIndex);
			System.out.println("Temp: "+decisionDateTemp);
			
			dateTemp = pdfFileContent.substring((decisionDateTemp.lastIndexOf("du")+1),endIndex-9);
			System.out.println("dateTemp "+dateTemp+"\n\n************");
			if(dateTemp.toLowerCase().contains("janvier".toLowerCase())){
				decisionDate = dateTemp.replaceAll("janvier", "January");
			}
			if(dateTemp.toLowerCase().contains("février".toLowerCase())){
				decisionDate = dateTemp.replaceAll("février", "February");
			}
			if(dateTemp.toLowerCase().contains("mars".toLowerCase())){
				decisionDate = dateTemp.replaceAll("mars", "March");
			}
			if(dateTemp.toLowerCase().contains("avril".toLowerCase())){
				decisionDate = dateTemp.replaceAll("avril", "April");
			}
			if(dateTemp.toLowerCase().contains("mai".toLowerCase())){
				decisionDate = dateTemp.replaceAll("mai", "May");
			}
			if(dateTemp.toLowerCase().contains("juin".toLowerCase())){
				decisionDate = dateTemp.replaceAll("juin", "June");
			}
			if(dateTemp.toLowerCase().contains("juillet".toLowerCase())){
				decisionDate = dateTemp.replaceAll("juillet", "July");
			}
			if(dateTemp.toLowerCase().contains("août".toLowerCase())){
				decisionDate = dateTemp.replaceAll("août", "August");
			}
			if(dateTemp.toLowerCase().contains("septembre".toLowerCase())){
				decisionDate = dateTemp.replaceAll("septembre", "September");
			}
			if(dateTemp.toLowerCase().contains("octobre".toLowerCase())){
				decisionDate = dateTemp.replaceAll("octobre", "October");
			}
			if(dateTemp.toLowerCase().contains("novembre".toLowerCase())){
				decisionDate = dateTemp.replaceAll("novembre", "November");
			}
			if(dateTemp.toLowerCase().contains("décembre".toLowerCase())){
				decisionDate = dateTemp.replaceAll("décembre", "December");
			}
			System.out.println("Decison date: "+decisionDate);
