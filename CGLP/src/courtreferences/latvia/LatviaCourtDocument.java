...
public class LatviaCourtDocument extends CourtDocument implements ProcessCitations {
	/*
	...
	public LatviaCourtDocument(){
	}
	
	public LatviaCourtDocument(String countryName, String courtName, String processedUserName, String sourceFileName){
	...
	public void extractCNSTCRTCitations(List<String> fileContent){
		...
		for(String pageContent : fileContent){
			...
			while(true){
				...
				LatviaCitations currentCitationObj = new LatviaCitations(this.getCaseId(),this.getCountryName(),this.getCourtName(),currentCitationId,currCitationString.trim(),currentPageNo);
			...
			PageContent currentPage = new LatviaPageContent(currentPageNo, pageContent.toLowerCase());
	...
	public void extractCitationsFromPageBodyContent(){
		for(PageContent bodyContent : this.getDocumentPages()){
			LatviaCitations citationObj = new LatviaCitations();
	...
	int checkCaseIdPattern(String inputString, int startIndex, int option){
		String caseidPatternString = "([c][a][s][e]\\s[N][o]\\.\\s?\\d{2,4}[-]\\.?\\d{2}[-]?[(]?\\d{2,4}[)]?)";
